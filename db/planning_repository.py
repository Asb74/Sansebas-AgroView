...
class PlanningRepository:
    ...
    def _get_harvestsync_template(self, collection: str, cultivo: str) -> list[str]:
        ...
        if client is None:
            return []
        try:
            snap = client.collection(collection).document(cultivo_norm).get()
            data = snap.to_dict() if getattr(snap, "exists", False) else {}
            campo = (data or {}).get("CAMPO") or []
    ...
    def _get_harvestsync_aprovechamiento_por_partida(self, partida: dict[str, Any]) -> list[dict[str, Any]]:
        boleta = str(partida.get("Boleta", "") or "").strip()
        cultivo = str(partida.get("Cultivo", "") or "").strip().upper()
        fecha_dt = self._parse_date(partida.get("Fecha carga", partida.get("FechaCarga", "")))
        if not boleta or not cultivo or not fecha_dt:
            return []
        fecha_key = fecha_dt.strftime("%Y-%m-%d")
        cache_key = (cultivo, boleta, fecha_key)
        if cache_key in self._harvestsync_cache:
            return [dict(r) for r in self._harvestsync_cache[cache_key]]
        client = self._get_harvestsync_client()
        if client is None:
            self._harvestsync_cache[cache_key] = []
            return []
        desde = (fecha_dt - timedelta(days=3)).date()
        hasta = fecha_dt.date()
        ...
        campos_calibre = self._get_harvestsync_template("PlantillasCalibre", cultivo)
        if not campos_aprov or not campos_calibre:
            self._harvestsync_cache[cache_key] = []
            return []
        avg_aprov = {c: sum(normalizar_numero(self._harvestsync_get_field(m, c)) for m in muestras) / len(muestras) for c in campos_aprov}
        destrio = avg_aprov.get("Destrio", avg_aprov.get("Destrío", 0.0))
        industria = avg_aprov.get("Industria", 0.0)
        categoria_i = avg_aprov.get("Categoria I", avg_aprov.get("Categoría I", 0.0))
        categoria_ii = avg_aprov.get("Categoria II", avg_aprov.get("Categoría II", 0.0))
        tiene_categoria_comercial = any(
            self._harvestsync_get_field(m, "Categoria I") not in (None, "")
            or self._harvestsync_get_field(m, "Categoría I") not in (None, "")
            or self._harvestsync_get_field(m, "Categoria II") not in (None, "")
    ...
    def _get_campo_disponibilidad_aprovechamiento(self, stock_campo_rows: list[dict[str, Any]], filters: dict) -> tuple[list[dict[str, Any]], int]:
        ...
        boleta_to_albaranes = {
            boleta: albaranes_por_boleta.get(boleta, [])
            for boleta in boletas_sin_pesosfres
            if albaranes_por_boleta.get(boleta)
        }
        loteado_bulk = self._get_loteado_aprovechamiento_por_boleta_bulk(
            boleta_to_albaranes,
            filters or {},
            kg_campo_por_boleta,
            calidad_bulk,
        ...
        for boleta in boletas_sin_pesosfres:
            loteado_rows = loteado_bulk.get(boleta, [])
            if loteado_rows:
            ...
            for partida_hs in stock_por_boleta.get(boleta, [{"Boleta": boleta}]):
                harvestsync_rows = self._get_harvestsync_aprovechamiento_por_partida(partida_hs)
                if harvestsync_rows:
                    break
            if harvestsync_rows:
        ...
        out: list[dict[str, Any]] = []
        sin_datos = 0
        detalle_stats = {"pesosfres_partidas": 0, "loteado_con_volcado": 0, "loteado_sin_volcado": 0, "harvestsync_partidas": 0, "harvestsync_comercial_sum": 0.0, "manual_partidas": 0}
        used: set[str] = set()
        for partida in stock_campo_rows:
            boleta = str(partida.get("Boleta", "") or "").strip()
            ...
            for template in template_rows:
                ...
                if kg_estimado <= 0:
                    continue
                calibre_row = str(row.get("Calibre", "") or "").strip().upper()
                if calibre_row.startswith("CAL "):
                    row[f"{calibre_row} %"] = round(pct, 4)
                row.update({
                    "Tipo stock": "CAMPO",
            ...
            if "LOTEADO" in origenes_partida:
                if any(r.get("Destrío %") is not None or r.get("Industria %") is not None for r in partida_rows):
                    detalle_stats["loteado_con_volcado"] += 1
                else:
            ...
            if "ESTIMADO_MANUAL" in origenes_partida:
                detalle_stats["manual_partidas"] += 1
        ...
        hs_media = (detalle_stats["harvestsync_comercial_sum"] / hs_filas) if hs_filas else 0.0
        logger.info("[APROV DETALLE] PesosFres filas con destrio/industria=%s", detalle_stats["pesosfres_partidas"])
        logger.info("[APROV DETALLE] Loteado partidas con volcado=%s sin_volcado=%s", detalle_stats["loteado_con_volcado"], detalle_stats["loteado_sin_volcado"])
        logger.info("[APROV DETALLE] HarvestSync filas=%s comercial_pct_medio=%.1f", hs_filas, hs_media)
        logger.info("[APROV DETALLE] Manual filas=%s", detalle_stats["manual_partidas"])
    ...
    def _get_pesosfres_aprovechamiento_por_boleta(self, boleta: str, filters: dict) -> list[dict[str, Any]]:
        ...
        with sqlite3.connect(fruta_path) as conn:
            ...
            podrido_col = self._find_column(cols, ["Podrido"])
            deslinea_col = self._find_column(cols, ["DesLinea"])
            desmesa_col = self._find_column(cols, ["DesMesa"])
        ...
        if total_valido <= 0 or validas == 0:
            ...
            return []
        out = []
        destrio_pct = round(podrido_total / total_valido * 100, 4) if total_valido > 0 else 0.0
        industria_pct = round(industria_total / total_valido * 100, 4) if total_valido > 0 else 0.0
        comercial_pct = round(sum(sum_cal.values()) / total_valido * 100, 4) if total_valido > 0 else 0.0
        ...
        for calibre in sorted(sum_cal, key=lambda x: int(x.replace("CAL ", ""))):
            ...
            out.append({"Origen": "CAMPO_REAL_PESOSFRES", "Calibre": calibre, "Categoría": categoria or "NORMAL", "% aprovechamiento": pct_redondeado, "Destrío %": destrio_pct, "Industria %": industria_pct, "Comercial %": comercial_pct, f"{calibre} %": pct_redondeado, "Origen aprovechamiento": "REAL_PESOSFRES", "Aviso": f"Aprovechamiento real PesosFres boleta {boleta}".strip(), "Explicación": "Distribución porcentual media por calibre calculada desde entregas válidas de PesosFres; los kilos se aplican después a cada partida"})
    ...
    def _get_volcado_pcts_por_partida_incluida(self, ids_partida: list[str]) -> dict[str, dict[str, float]]:
        calidad_path = self._db_path(DB_CALIDAD)
        ids = [str(v or "").strip() for v in ids_partida if str(v or "").strip()]
        if not ids or not calidad_path.exists():
            return {}
        ...
        try:
            with sqlite3.connect(calidad_path) as conn:
                ...
                for idx in range(10):
                    ...
                    if not inc_col:
                        continue
                    sql = f'SELECT p."{inc_col}" AS Incluida, dc.* FROM "{p_table}" p JOIN "{dc_table}" dc ON TRIM(CAST(p."{idp_col}" AS TEXT)) = TRIM(CAST(dc."{id_dc}" AS TEXT)) WHERE TRIM(CAST(p."{inc_col}" AS TEXT)) <> ""'
    ...
    def _get_loteado_aprovechamiento_por_boleta_bulk(
        self,
        boleta_to_albaranes: dict[str, list[str]],
        filters: dict,
        kg_campo_por_boleta: dict[str, float],
        calidad_bulk: dict[str, dict],
    ) -> dict[str, list[dict[str, Any]]]:
        start = time.perf_counter()
        normalized: dict[str, list[str]] = {}
        albaran_to_boletas: dict[str, set[str]] = defaultdict(set)
        for boleta, albaranes in (boleta_to_albaranes or {}).items():
            boleta_norm = str(boleta or "").strip()
            if not boleta_norm:
                continue
            vistos: set[str] = set()
            for albaran in albaranes or []:
                albaran_norm = str(albaran or "").strip()
                if not albaran_norm or albaran_norm in vistos:
                    continue
                vistos.add(albaran_norm)
                albaran_to_boletas[albaran_norm].add(boleta_norm)
            if vistos:
                normalized[boleta_norm] = list(vistos)
        if not normalized or not self.db_loteado.exists():
            logger.info("[PERF LoteadoBulk.Total] boletas=%s albaranes=%s filas_lote=%s tiempo=%.2fs", len(normalized), len(albaran_to_boletas), 0, time.perf_counter() - start)
            return {}
        ...
        try:
            with sqlite3.connect(self.db_loteado) as conn:
                ...
                if not all([idpalet_ldo, idpalet_lote, idlote_col, neto_col, calibre_col]):
                    logger.info("[PERF LoteadoBulk.Total] boletas=%s albaranes=%s filas_lote=%s tiempo=%.2fs", len(normalized), len(albaran_to_boletas), 0, time.perf_counter() - start)
                    return {}
                conn.execute(f'CREATE INDEX IF NOT EXISTS idx_lote_idlote ON "{lote_table}"("{idlote_col}")')
                conn.execute(f'CREATE INDEX IF NOT EXISTS idx_lote_idpalet ON "{lote_table}"("{idpalet_lote}")')
                conn.execute(f'CREATE INDEX IF NOT EXISTS idx_loteado_idpalet ON "{ldo_table}"("{idpalet_ldo}")')
                logger.info("[PERF LoteadoBulk.Schema] tiempo=%.2fs", time.perf_counter() - schema_start)

                insert_start = time.perf_counter()
                conn.execute("DROP TABLE IF EXISTS temp_lotes_busqueda")
                conn.execute("CREATE TEMP TABLE temp_lotes_busqueda(id_lote TEXT PRIMARY KEY, boleta TEXT)")
                conn.executemany(
                    "INSERT OR IGNORE INTO temp_lotes_busqueda(id_lote, boleta) VALUES (?, ?)",
                    [(albaran, next(iter(boletas), "")) for albaran, boletas in albaran_to_boletas.items()],
                )
                logger.info("[PERF LoteadoBulk.InsertTemp] albaranes=%s tiempo=%.2fs", len(albaran_to_boletas), time.perf_counter() - insert_start)
                ...
                def fetch_rows(legacy: bool) -> list[sqlite3.Row]:
                    where = [f'CAST(lote."{neto_col}" AS REAL) > 0']
                    params: list[Any] = []
                    calibre_expr = f'TRIM(CAST(lote."{calibre_col}" AS TEXT))' if legacy else f'lote."{calibre_col}"'
                    where.append(f'{calibre_expr} <> ""')
                    for values, column, upper in filter_specs:
                        values = [v for v in values if v.upper() != "TODOS"]
                        if values and column:
                            ...
                            params.extend(values)
                    if legacy:
                        join_palet = f'TRIM(CAST(lote."{idpalet_lote}" AS TEXT)) = TRIM(CAST(ldo."{idpalet_ldo}" AS TEXT))'
                        join_lote = f'TRIM(CAST(lote."{idlote_col}" AS TEXT)) = temp.id_lote'
                    else:
                        join_palet = f'lote."{idpalet_lote}" = ldo."{idpalet_ldo}"'
                        join_lote = f'lote."{idlote_col}" = temp.id_lote'
                    sql = (
                        f'SELECT lote."{idpalet_lote}" AS IdPalet, lote."{idlote_col}" AS IdLote, '
                        f'lote."{neto_col}" AS Neto, lote."{calibre_col}" AS Calibre, {cat_expr} AS Categoria '
                        f'FROM temp_lotes_busqueda temp '
                        f'JOIN "{lote_table}" lote ON {join_lote} '
                        f'JOIN "{ldo_table}" ldo ON {join_palet} '
                        f'WHERE {" AND ".join(where)}'
                    )
                    return conn.execute(sql, params).fetchall()

                query_start = time.perf_counter()
                rows = fetch_rows(legacy=False)
                if not rows:
                    rows = fetch_rows(legacy=True)
                    logger.info("[PERF LoteadoBulk.Query] ruta=fallback_lenta filas=%s tiempo=%.2fs", len(rows), time.perf_counter() - query_start)
                else:
                    logger.info("[PERF LoteadoBulk.Query] ruta=rapida filas=%s tiempo=%.2fs", len(rows), time.perf_counter() - query_start)

                process_start = time.perf_counter()
                for row in rows:
                    filas_lote += 1
                    id_lote = str(row["IdLote"] or "").strip()
                    id_palet = str(row["IdPalet"] or "").strip()
                    dedup_key = (id_palet, id_lote)
                    ...
                    for boleta in albaran_to_boletas.get(id_lote, set()):
                        for calibre, kg_cal in reparto.items():
                            grouped[boleta][(calibre, categoria)] += kg_cal
                            totals[boleta] += kg_cal
                logger.info("[PERF LoteadoBulk.ProcessRows] filas=%s tiempo=%.2fs", filas_lote, time.perf_counter() - process_start)
        except Exception as exc:
            logger.warning("No se pudo consultar loteado bulk: %s", exc)
            return {}

        out: dict[str, list[dict[str, Any]]] = {}
        for boleta, total in totals.items():
            if total <= 0:
                continue
            ...
            distribucion_pct: dict[str, float] = {}
            for calibre, categoria in sorted(grouped[boleta], key=lambda x: (int(x[0].replace("CAL ", "")) if str(x[0]).startswith("CAL ") else 99, x[1])):
                kg_cal = grouped[boleta][(calibre, categoria)]
                if kg_cal <= 0:
                    continue
    ...
    def _get_loteado_aprovechamiento_por_boleta(self, boleta: str, albaranes_boleta: list[str], filters: dict, calidad_pcts: dict[str, float] | None = None) -> list[dict[str, Any]]:
        ...
        if calidad_pcts is None:
            calidad_pcts = self._get_loteado_calidad_pcts_por_id_lotes(albaranes_boleta)
        calidad_bulk: dict[str, dict] = {boleta: calidad_pcts} if calidad_pcts else {}
    ...
    def _get_loteado_campo_disponibilidad_real(
        ...
        start = time.perf_counter()
        loteado_rows: list[dict[str, Any]] = []
        ...
        def crear_indices(conn: sqlite3.Connection, statements: list[str]) -> None:
        ...
        if not pendientes:
            ...
            return [], 0
        try:
            albaran_to_boleta: dict[str, str] = {}
            with sqlite3.connect(fruta_path) as conn_fruta:
                conn_fruta.row_factory = sqlite3.Row
                crear_indices(
                    conn_fruta,
                    [
                        'CREATE INDEX IF NOT EXISTS idx_pesosfres_boleta ON PesosFres(Boleta)',
                        'CREATE INDEX IF NOT EXISTS idx_pesosfres_albarandef ON PesosFres(AlbaranDef)',
                    ],
                )
                pf_cols = [r[1] for r in conn_fruta.execute('PRAGMA table_info("PesosFres")').fetchall()]
                albaran_col = self._find_column(pf_cols, ["AlbaranDef"])
                boleta_col = self._find_column(pf_cols, ["Boleta"])
                if not albaran_col or not boleta_col:
                    logger.warning("Columnas insuficientes en PesosFres para aprovechamiento por loteado")
                    log_loteado_exit()
                    return [], len(pendientes)
                chunk_size = 500
                for i in range(0, len(boletas_pendientes), chunk_size):
                    ...
                    placeholders = ",".join(["TRIM(CAST(? AS TEXT))"] * len(chunk))
                    sql = (
                        f'SELECT DISTINCT TRIM(CAST("{albaran_col}" AS TEXT)) AS AlbaranDef, '
                        f'TRIM(CAST("{boleta_col}" AS TEXT)) AS Boleta '
                        f'FROM "PesosFres" '
                        f'WHERE TRIM(CAST("{albaran_col}" AS TEXT)) <> "" '
                        f'AND TRIM(CAST("{boleta_col}" AS TEXT)) IN ({placeholders})'
                    )
                    for row in conn_fruta.execute(sql, chunk).fetchall():
                        albaran = str(row["AlbaranDef"] or "").strip()
                        boleta = str(row["Boleta"] or "").strip()
            ...
            volcado_pcts_por_albaran = self._get_volcado_pcts_por_partida_incluida(list(albaran_to_boleta))
            volcado_pcts_por_boleta = {albaran_to_boleta[k]: v for k, v in volcado_pcts_por_albaran.items() if k in albaran_to_boleta}
            if not albaran_to_boleta:
                log_loteado_exit(0)
                return [], len(pendientes)

            albaranes_pendientes = sorted(albaran_to_boleta)
            with sqlite3.connect(loteado_path) as conn_loteado:
                conn_loteado.row_factory = sqlite3.Row
                crear_indices(
                    conn_loteado,
                    [
                        'CREATE INDEX IF NOT EXISTS idx_lote_idlote ON Lote(IdLote)',
                        'CREATE INDEX IF NOT EXISTS idx_lote_idpalet ON Lote(IdPalet)',
                        'CREATE INDEX IF NOT EXISTS idx_loteado_idpalet ON Loteado(IdPalet)',
                        'CREATE INDEX IF NOT EXISTS idx_loteado_campana ON Loteado(CAMPAÑA)',
                        'CREATE INDEX IF NOT EXISTS idx_loteado_cultivo ON Loteado(CULTIVO)',
                        'CREATE INDEX IF NOT EXISTS idx_loteado_empresa ON Loteado(EMPRESA)',
                    ],
                )
                ldo_table = self._find_table(conn_loteado, ["Loteado", "loteado", "LOTEADO"])
                lote_table = self._find_table(conn_loteado, ["Lote", "lote", "LOTE"])
                if not ldo_table or not lote_table:
                    logger.warning("No existe tabla Loteado o Lote para aprovechamiento por loteado")
                    log_loteado_exit()
                    return [], len(pendientes)
                ldo_cols = [r[1] for r in conn_loteado.execute(f'PRAGMA table_info("{ldo_table}")').fetchall()]
                lote_cols = [r[1] for r in conn_loteado.execute(f'PRAGMA table_info("{lote_table}")').fetchall()]
                idpalet_ldo = self._find_column(ldo_cols, ["IdPalet"])
                idpalet_lote = self._find_column(lote_cols, ["IdPalet"])
                idlote_col = self._find_column(lote_cols, ["IdLote"])
                neto_col = self._find_column(lote_cols, ["Neto"])
                ...
                for values, column, use_upper in ((campanas, campana_col, False), (cultivos, cultivo_col, True), (empresas, empresa_col, False)):
                    if values and column:
                        placeholders = ",".join(["TRIM(CAST(? AS TEXT))"] * len(values))
                        expr = f'UPPER(TRIM(CAST(ldo."{column}" AS TEXT)))' if use_upper else f'TRIM(CAST(ldo."{column}" AS TEXT))'
                        where_base.append(f'{expr} IN ({placeholders})')
                        params_base.extend(values)
                cat_expr = f'lote."{categoria_col}"' if categoria_col else "'NORMAL'"
                select_sql = (
                    f'SELECT lote."{idpalet_lote}" AS IdPalet, '
                    f'lote."{idlote_col}" AS IdLote, lote."{neto_col}" AS Neto, '
                    f'lote."{calibre_col}" AS Calibre, {cat_expr} AS Categoria '
                    f'FROM "{lote_table}" lote JOIN "{ldo_table}" ldo '
                    f'ON TRIM(CAST(lote."{idpalet_lote}" AS TEXT)) = TRIM(CAST(ldo."{idpalet_ldo}" AS TEXT))'
                )
                for i in range(0, len(albaranes_pendientes), chunk_size):
                    chunk = albaranes_pendientes[i:i + chunk_size]
                    lote_placeholders = ",".join(["TRIM(CAST(? AS TEXT))"] * len(chunk))
                    where = where_base + [f'TRIM(CAST(lote."{idlote_col}" AS TEXT)) IN ({lote_placeholders})']
                    sql = f'{select_sql} WHERE {" AND ".join(where)}'
                    for row in conn_loteado.execute(sql, params_base + chunk).fetchall():
                        item = dict(row)
                        id_lote = str(item.get("IdLote", "") or "").strip()
                        boleta = albaran_to_boleta.get(id_lote)
                        if boleta:
                            item["Boleta"] = boleta
        ...
        for r in rows:
            ...
            if id_palet:
                palets_by_boleta[boleta].add(id_palet)
            if id_lote:
                lotes_by_boleta[boleta].add(id_lote)
            ...
            logger.debug("Loteado encontrado por boleta=%s kg=%s calibre=%s reparto=%s", boleta, kg, r.get("Calibre"), reparto)
            for calibre, kg_cal in reparto.items():
        ...
        for boleta, total in total_by_boleta.items():
            ...
            for (b, calibre, categoria), kg_cal in grouped.items():
                if b == boleta and kg_cal > 0:
                    row_out = {"Origen": "CAMPO_REAL_LOTEADO", "Tipo stock": "CAMPO", "Cultivo": partida.get("Cultivo", ""), "Campaña": partida.get("Campaña", partida.get("Campana", "")), "Grupo varietal": partida.get("Grupo varietal", ""), "Variedad": partida.get("Variedad", ""), "Calibre": calibre, "Categoría": categoria, "Kg disponibles": round(kg_cal, 2), "Kg campo origen": kg_campo, "Boleta": boleta, "Socio": partida.get("Socio", ""), "Fecha carga": partida.get("Fecha carga", partida.get("FechaCarga", "")), "% aprovechamiento": round((kg_cal / kg_campo * 100) if kg_campo > 0 else 0, 4), "Origen aprovechamiento": "LOTEADO", "Aviso": f"Aprovechamiento calculado desde loteado boleta {boleta}".strip(), "Explicación": "Kg estimados por calibre calculados desde netos loteados vinculados a PesosFres"}
                    row_out.update(volcado_pcts_por_boleta.get(boleta, {}))
                    out.append(row_out)
        log_loteado_exit()
