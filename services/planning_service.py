...
class PlanningService:
    ...
    def export_rows_to_excel(self, rows: list[dict], tab_name: str, cultivos: list[str], campanas: list[str]) -> str | None:
        ...
        for i, h in enumerate(headers, start=1):
            if h in ("Kg campo", "Kg stock", "Kg pedido teórico", "Kg hecho real", "Kg pendiente", "Merma kg", "Kg stock comercial", "Kg pedidos pendientes", "Diferencia comercial", "Kg stock industrial almacén", "Kg campo estimado", "Kg industrial total", "Kg cobertura exacta", "Kg cobertura agrupada", "Kg cobertura potencial total"):
